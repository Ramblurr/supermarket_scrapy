...
class ReweShopSider(abstractShopSpider.AbstractShopSpider, scrapy.Spider):
    ...
    start_url = (
        "https://shop.rewe.de/productList"
    )  # this is no typo! See def start_requests
        ...
        if nextLink:
            ...
            if nextLink in self.visited:
                ...
                message = """{time}\n
                           ...
                           \t {number} - times\n
                           """.format(
                    time=thisTime, plz=plz, url=nextLink, number=self.visited[nextLink]
                )
               ...
               \t {url} \n
               """.format(
                    time=thisTime, plz=plz, url=nextLink
                )
            ...
            if self.visited[nextLink] <= self.allowedVisits:
                request = scrapy.Request(
                    nextLink, callback=self.parse, meta={"plz": plz}, dont_filter=True
                )  # !!!

                yield request

        productLinks = response.css(
            "div.MainTypeCompOrgaProdProductTileContent a::attr(href)"
        )
    ...
    def getIngredients(self, response=None, data=None):
        ingredientString = response.xpath(
            '//h3[contains(., "Zutaten:")]/parent::div/text()'
        )
