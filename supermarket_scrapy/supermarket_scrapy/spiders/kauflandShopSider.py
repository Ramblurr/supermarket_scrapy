...
class KauflandShopSpider(abstractShopSpider.AbstractShopSpider, scrapy.Spider):
    name = "KauflandShop"
    start_urls = [
        "https://shop.kaufland.de/search?sort=relevance&pageSize=48&source=search"
    ]
    ...
    def getIngredients(self, response=None, data=None):
        ingredientString = response.xpath(
            './/h4[.="Zutatenliste"]/following-sibling::p/text()'
        )
    ...
    def getProducer(self, response=None, data=None):
        producer = response.xpath(
            './/h4[.="Name Hersteller"]/following-sibling::p/text()'
        )
    ...
    def getPrice(self, response=None, data=None):
        ...
        price = response.xpath(
            '//div[contains(@class,"product-info--regularprice")]/text()'
        )
