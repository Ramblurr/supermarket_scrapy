...
class AmazonLebensmittelShopSpider(
    abstractShopSpider.AbstractShopSpider, scrapy.Spider
):
    ...
    ingridientRegexes = [re.compile('zutaten:([\s\w"\%\!\,\.äüöß*]+)', re.IGNORECASE)]
    ingredientsRegExCleaners = [re.compile("\s.*", re.DOTALL), re.compile("[\W]")]
    ...
    def getSize(self, response=None, data=None):
        returnDict = dict()
        size = response.xpath(
            '//td[contains(@class, "label") and contains(., "Gewicht")]/following-sibling::td[contains(@class, "value")]/text()'
        )
