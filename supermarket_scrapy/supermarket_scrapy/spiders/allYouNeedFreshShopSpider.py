...
class AllYouNeedFreshShopSpider(abstractShopSpider.AbstractShopSpider, scrapy.Spider):
    ...
    start_url = (
        "https://www.allyouneedfresh.de/alle_kategorien"
    ...
    def start_requests(self):
        for plz in self.mainPLZ.values():
            request = scrapy.Request(
                self.start_url, callback=self.parse, meta={"plz": plz}, dont_filter=True
            )  # !!!
            yield request
        ...
        for menuLink in menuLinks:
            ...
            if menuLink in self.visited:
                ...
                message = """{time}\n
                           ...
                           \t {number} - times\n
                           """.format(
                    time=thisTime, plz=plz, url=menuLink, number=self.visited[menuLink]
                )
                ...
                message = """{time}\n
                           ...
                           \t {url} \n
                           """.format(
                    time=thisTime, plz=plz, url=menuLink
                )
                self.logger.debug(message)
            if self.visited[menuLink] <= self.allowedVisits:
                request = scrapy.Request(
                    menuLink, callback=self.parseCategory, dont_filter=True  # !!!
                )
    ...
    def parseCategory(self, response):
        ...
        for productLink in productLinks:
            ...
            if productLink not in self.parsedProducts:
                ...
                request = scrapy.Request(
                    productLink, callback=self.parseProduct, dont_filter=True  # !!!
                )
                request.meta["plz"] = plz
                yield request
                ...
                message = """°°°°°°°°°°°°°°°°°°°°\n
                             ...
                             °°°°°°°°°°°°°°°°°°°°\n
                             """.format(
                    time=thisTime, plz=plz, url=productLink
                )
