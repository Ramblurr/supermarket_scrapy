...
class LebensmittelShopSpider(abstractShopSpider.AbstractShopSpider, scrapy.Spider):
    ...
    getParamRegEx = re.compile(
        "https:\/\/www\.lebensmittel\.de\/\?p=produktliste&CatLevel=1&subd=0&CatID=(\d+)"
    )
    ...
    biologischerAnbauRegEx = re.compile(
        "aus kontrolliert biologischem anbau", flags=re.IGNORECASE
    )
    ...
    def parse(self, response):
        ...
        for categoryLink in categoryLinks:
            ...
            url = self.productListLink.format(start=0, end=101, page=0, CatID=CatID)
    ...
    def parseProductList(self, response):
        ...
        if productLinks:
            for productLink in productLinks:
                ...
                yield response.follow(productLink, callback=self.preParseProduct)
        ...
        for page in range(1, pages):  # We have 0 allready
            ...
            url = self.productListLink.format(
                start=start, end=end, page=page, CatID=CatID
            )
